...
export default function LiveFeed() {
  ...
  return (
    <div className="flex h-[calc(100vh-72px)] w-full bg-slate-950 text-slate-200 font-sans overflow-hidden">
      ...
      <div className="w-2/3 flex flex-col border-r border-slate-800 p-8 relative">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-4">
          {cards.length === 0 && !isLive && (
            <div className="text-center py-12 text-slate-500">
              <Radio className="w-16 h-16 mx-auto mb-4 opacity-30" />
              <p className="text-lg">Click "ACTIVATE SCANNER" to start real-time monitoring</p>
            </div>
          )}
          {cards.map((card, i) => (
            <div 
              key={i} 
              className="bg-slate-900 border-l-4 border-red-500 p-6 rounded shadow-lg animate-in slide-in-from-top-4 transition-all duration-500"
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 border transition-colors duration-500 ${
              ...
              <p className="text-slate-300 mb-4 bg-slate-950/50 p-3 rounded border border-slate-800 font-mono text-sm">
                "{card.details}"
              </p>

              <div className="flex items-center gap-4 text-sm mt-4 pt-4 border-t border-slate-800">
                <div className="flex items-center gap-2 text-yellow-500">
                  <AlertTriangle className="w-4 h-4" /> URGENCY: {card.urgency}
                </div>
                <div className="flex items-center gap-2 text-blue-400">
                  <Users className="w-4 h-4" /> Sources: {card.source_count}
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="w-1/3 bg-black p-6 font-mono text-xs text-green-500/80 flex flex-col border-l border-slate-800">
        <div className="border-b border-green-900/50 pb-2 mb-4 font-bold flex justify-between">
          ...
          <span className={isLive ? 'animate-pulse' : ''}>{isLive ? '● REC' : '○ IDLE'}</span>
        </div>
        <div className="space-y-2 overflow-y-auto flex-1">
          {logs.length === 0 && (
            <div className="text-green-900">Waiting for activation...</div>
          )}
          {logs.map((log, i) => (
